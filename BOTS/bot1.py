...
def safe_parse_dt(val: Any) -> datetime | None:
    """Parse string or datetime to a naive datetime object safely."""
    ...
    if isinstance(val, str):
        ...
        if not val_clean:
            return None
        try:
            return datetime.fromisoformat(val_clean.replace("Z", "+00:00")).replace(tzinfo=None)
        except Exception:
            pass
...
def safe_format_date(val: Any, fmt: str = "%B %Y", default: str = "Recently") -> str:
    """Safely format a date whether it is datetime, ISO string, timestamp, or None."""
...
# ==========================================
# 📊 DATABASE CONNECTION & ENTERPRISE CONNECTOR
# ==========================================
client = None
db = None
backup_client_b1 = None
backup_db_b1 = None
col_user_verification = None
col_msa_ids = None
col_pdfs = None
col_ig_content = None
col_support_tickets = None
col_banned_users = None
col_suspended_features = None
col_bot1_settings = None
col_referrals = None
col_reviews = None
col_rewards = None
col_live_logs = None
col_msa_credits = None
col_store_items = None
col_milestones = None
col_broadcasts = None
col_bot1_backups = None
col_bot1_restore_data = None

def connect_db(is_reconnect: bool = False) -> bool:
    """
    Connect or reconnect to MongoDB clusters with enterprise resilience.
    Sets up client, backup_client_b1, db, and binds all collection handles globally.
    """
    global client, db, backup_client_b1, backup_db_b1
    global col_user_verification, col_msa_ids, col_pdfs, col_ig_content
    global col_support_tickets, col_banned_users, col_suspended_features
    global col_bot1_settings, col_referrals, col_reviews, col_rewards
    global col_live_logs, col_msa_credits, col_store_items, col_milestones
    global col_broadcasts, col_bot1_backups, col_bot1_restore_data

    try:
        import certifi
        # Close stale client if reconnecting
        if is_reconnect and client is not None:
            try:
                client.close()
            except Exception:
                pass
        ...
        if new_db.name != "MSANodeDB":
            logger.critical(f"❌ FATAL: MONGO_DB_NAME is '{new_db.name}' — must be 'MSANodeDB'. Fix your env vars and restart.")
            if not is_reconnect:
                sys.exit(1)
            return False
        ...
        # Bind all primary collections
        col_user_verification = db["bot1_user_verification"]
        col_msa_ids = db["bot1_msa_ids"]
        col_pdfs = db["bot3_pdfs"]
        col_ig_content = db["bot3_ig_content"]
        col_support_tickets = db["bot1_support_tickets"]
        col_banned_users = db["bot1_banned_users"]
        col_suspended_features = db["bot1_suspended_features"]
        col_bot1_settings = db["bot1_settings"]
        col_referrals = db["bot1_referrals"]
        col_reviews = db["bot1_reviews"]
        ...
        if not BACKUP_MONGO_URI:
            ...
            if is_reconnect and backup_client_b1 is not None and backup_client_b1 is not client:
                try:
                    backup_client_b1.close()
                except Exception:
                    pass
            try:
                ...
                backup_client_b1.admin.command('ping')
            except Exception as _bke:
                logger.warning(f"⚠️ Backup cluster ping failed ({_bke}) — falling back to main client")
                backup_client_b1 = client

        backup_db_b1 = backup_client_b1[_bk_db]
        col_bot1_backups = backup_db_b1["bot1_backups"]
        col_bot1_restore_data = backup_db_b1["bot1_restore_data"]

        if is_reconnect:
            health_stats["db_reconnects"] += 1
            logger.info(f"✅ MongoDB auto-reconnected successfully (reconnect count: {health_stats['db_reconnects']})")
        else:
...
if not connect_db(is_reconnect=False):
    sys.exit(1)
    
    # ==========================================
    # 🔍 CREATE DATABASE INDEXES (Performance & Dedup Constraints)
    # ==========================================
    def init_database_indexes():
        """Create and verify all database indexes and deduplication constraints."""
        try:
            ...
            col_bot1_settings.create_index("setting", unique=True)
            db["bot1_permanently_banned_msa"].create_index("user_id", unique=True)
            db["bot1_permanently_banned_msa"].create_index("msa_id", unique=True)
...
def track_user_source(user_id: int, source: str, username: str, first_name: str, msa_id: str, language_code: str = "unknown"):
    """
    ...
    try:
        ...
        if existing is None:
            ...
            if current_is_specific:
                ...
                col.update_one(
                    {"user_id": user_id},
                    {"$set": {"last_start": now_local(), "username": username, "first_name": first_name, "language_code": language_code}}
                )
...
def check_ticket_rate_limit(user_id: int, user_name: str = "You") -> tuple[bool, str]:
    ...
    if last_ticket:
        last_at = safe_parse_dt(last_ticket.get("created_at"))
        if last_at and last_at > cutoff:
            ...
            unlock_at = safe_format_date(unlock_dt, "%I:%M:%S %p", "Soon")
...
async def check_maintenance_mode(message: types.Message) -> bool:
    ...
    try:
        ...
        if settings and settings.get("value", False):
            # Check Testing Mode Bypass
            if settings.get("testing_mode", False):
                test_ids = settings.get("test_ids", [])
                if user_id in test_ids:
                    return False  # Whitelisted tester bypasses maintenance mode
...
async def _send_vault_hub(message: types.Message, state: FSMContext, uid: int, uname: str):
    ...
    for i, item in enumerate(owned_items, 1):
        ...
        date_str = safe_format_date(buy_entry["at"], "%b %d, %Y", "N/A") if buy_entry and buy_entry.get("at") else "N/A"
        item_lines.append(
            f"<b>{i}. {itm_name}</b>\n"
...
async def handle_vault_browsing(message: types.Message, state: FSMContext):
    ...
    date_str = safe_format_date(buy_entry["at"], "%b %d, %Y  %I:%M %p", "N/A") if buy_entry and buy_entry.get("at") else "N/A"
...
async def rate_agent_handler(message: types.Message, state: FSMContext):
    ...
    if existing:
        ...
        if _last_nv and (_now_nv - _last_nv) < timedelta(days=365):
            ...
            _past_date = safe_format_date(_last, '%b %d, %Y', 'Recently')
...
async def confirm_referral(referred_id: int, referred_name: str) -> None:
    ...
    try:
        ...
        if recent_hold:
            ...
            conf_at = safe_parse_dt(recent_hold.get("confirmed_at")) or now_local()
            next_slot = conf_at + timedelta(hours=24)
            next_fmt  = safe_format_date(next_slot, "%b %d  %I:%M %p")
    ...
    if ban_doc:
        ...
        if ban_type == "temporary" and ban_doc.get("ban_expires"):
            ...
            ban_message = (
                ...
                f"🕐 **Ban Start:** {safe_format_date(banned_at, '%b %d at %I:%M %p')}\n"
                f"🕐 **Ban Expires:** {safe_format_date(ban_expires, '%b %d at %I:%M %p')}\n"
            ...
            ban_message = (
                ...
                f"🕐 **Banned:** {safe_format_date(banned_at, '%b %d, %Y at %I:%M:%S %p')}\n\n"
    ...
    if old_status in ["left", "kicked"] and new_status in ["member", "administrator", "creator"]:
        ...
        try:
            ...
            if settings and settings.get("value", False) and user_id != OWNER_ID:
                ...
                track_user_source(user_id, _v_source, username, user_name, _msa_mm, getattr(event.from_user, "language_code", "unknown"))
                try:
                    await bot.send_message(
                        user_id,
                        f"👤 **Dear {user_name},**\n\n"
        ...
        track_user_source(user_id, _stored_source, username, user_name, msa_id, getattr(event.from_user, "language_code", "unknown"))
...
_ANN_CAP             = 5     # show only the N most recent broadcasts (top 5 latest)
...
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║                     BOT 1 — BROADCAST DISPLAY MODULE                       ║
# ║  Purpose: Reads bot2_broadcasts collection to render announcements live     ║
# ║  in the user-facing Bot 1 dashboard. Fully read-only — Bot 2 is the        ║
# ║  sole writer. Bot 1 never writes to bot2_broadcasts.                       ║
# ╚══════════════════════════════════════════════════════════════════════════════╝

# ── Broadcast Constants ───────────────────────────────────────────────────────
# _ANN_CAP         : Maximum number of recent broadcasts to load from DB.
# _ANN_PAGE_MAX_CHARS : Hard cap for text preview per announcement card.
# Both are defined earlier in the constants block.

def _fetch_deduplicated_broadcasts() -> list:
    """
    Fetch the _ANN_CAP (5) most-recent broadcasts from MSANodeDB.bot2_broadcasts.
    De-duplicated by broadcast_id (guards against any duplicate inserts).
    Sorted newest-first by sent_at / created_at. Returns at most _ANN_CAP items.

    Fields fetched (projection — keeps query light):
      - broadcast_id   : Unique ID (e.g. brd1, schbrd1)
      - created_at     : Datetime when broadcast was created / fired
      - sent_at        : Datetime when broadcast was sent
      - message_text   : Main text body of the broadcast
      - message        : Text fallback
      - media_type     : photo / video / text / etc.
      - br_index       : Normal broadcast index
      - sch_brd_index  : Scheduled broadcast index
      - index          : Legacy normal index
      - sched_index    : Legacy scheduled index
    """
    seen_ids: set = set()
    result = []
    try:
        cursor = (
            col_broadcasts.find(
                {"status": {"$in": ["sent", "completed", "fired", "sending"]}},
                {
                    "broadcast_id": 1,
                    "created_at": 1,
                    "sent_at": 1,
            ...
            .sort([("sent_at", -1), ("created_at", -1), ("br_index", -1), ("sch_brd_index", -1), ("index", -1)])
            .limit(_ANN_CAP * 4)
            .max_time_ms(4000)
        )
        for b in cursor:
...
def _build_ann_page(broadcasts: list, page: int) -> str:
    """
    Render a single ANNOUNCEMENTS card for the Bot 1 user dashboard.

    Args:
      broadcasts : De-duplicated list from _fetch_deduplicated_broadcasts().
      page       : Zero-based index into the list (wraps around safely).

    Behaviour:
      - Reads full broadcast text — no aggressive preview truncation.
      - Applies a hard character cap (_ANN_PAGE_MAX_CHARS) to prevent oversized cards.
      - Shows a type badge (📷 Photo / 🎥 Video / 📝 Text / etc.).
      - Shows a 🆕 badge for broadcasts created within the last 48 hours.
      - Caller should always pass a fresh query result (reflects live DB data).
    """
    if not broadcasts:
        return (
            "📢 **ANNOUNCEMENTS**\n"
    ...
    created_at = b.get("created_at") or b.get("sent_at")
    raw_text_value = b.get("message_text") or b.get("message")
    raw_text   = str(raw_text_value).strip() if raw_text_value is not None else ""
    media_type = b.get("media_type", "")
    b_index    = b.get("br_index") if b.get("br_index") is not None else (b.get("sch_brd_index") if b.get("sch_brd_index") is not None else b.get("index", page + 1))

    date_str = safe_format_date(created_at, "%b %d, %Y  ·  %I:%M %p", "—")
...
def _get_user_milestone_progress(ref_count: int) -> str:
    """Fetch bot3_milestones and build milestone tier & progress string for Bot 1 UI."""
    try:
        milestones = list(col_milestones.find({}).sort("refs_required", 1))
        if not milestones:
            return ""
        current_tier = None
        next_tier    = None
        for ms in milestones:
            req = ms.get("refs_required", 0)
            if req <= ref_count:
                current_tier = ms
            elif next_tier is None and req > ref_count:
                next_tier = ms
                break
        lines = []
        if current_tier:
            badge = current_tier.get("badge", "⭐")
            name  = current_tier.get("name", "Tier")
            lines.append(f"🏆 **Rank:** {badge} **{_escape_md(name)}** ({ref_count} refs)")
        else:
            lines.append(f"🏆 **Rank:** 👶 **Starter** ({ref_count} refs)")
            
        if next_tier:
            n_badge = next_tier.get("badge", "🎯")
            n_name  = next_tier.get("name", "Next Tier")
            n_req   = next_tier.get("refs_required", 0)
            needed  = n_req - ref_count
            lines.append(f"🎯 **Next Rank:** {n_badge} **{_escape_md(n_name)}** at **{n_req}** refs _({needed} more needed)_")
        else:
            lines.append("🌟 **Rank Status:** 👑 **MAX TIER REACHED!**")
        return "\n   ".join(lines)
    except Exception:
        return ""
...
def _build_dashboard_text(user_name, display_msa_id, member_since, ann_text, referral_stats=None, msa_credits=0) -> str:
    ...
    if referral_stats:
        ...
        ms_progress = _get_user_milestone_progress(confirmed)
        ms_line = f"   {ms_progress}\n" if ms_progress else ""
        ...
        referral_block = (
            ...
            f"{ms_line}"
            f"   🔗 `{short_link}`\n"
            f"   _(Share your link — earn rewards & unlock tier badges)_\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        )
    else:
        referral_block = ""
...
async def dashboard(message: types.Message):
    ...
    try:
        msa_record = col_msa_ids.find_one({"user_id": user_id})
        if msa_record and msa_record.get("assigned_at"):
            member_since = safe_format_date(msa_record["assigned_at"], "%B %Y", "Recently")
        else:
            user_data = col_user_verification.find_one({"user_id": user_id})
            if user_data and user_data.get("first_start"):
                member_since = safe_format_date(user_data["first_start"], "%B %Y", "Recently")
...
async def raise_ticket_handler(message: types.Message, state: FSMContext):
    ...
    if existing_ticket:
        ...
        date_str = safe_format_date(ticket_date, "%B %d, %Y at %I:%M %p", "Recently")
...
async def process_ticket_submission(message: types.Message, state: FSMContext):
    ...
    if open_ticket:
        ...
        created_text = safe_format_date(created_at, "%B %d, %Y at %I:%M %p", "Recently")
        await state.clear()
        await message.answer(
...
async def my_ticket_handler(message: types.Message):
    ...
    if open_ticket:
        created_at     = open_ticket.get("created_at", now_local())
        date_str       = safe_format_date(created_at, "%B %d, %Y at %I:%M %p", "Recently")
...
def _build_ticket_history_page(tickets: list, page: int, first_name: str) -> str:
    ...
    resolved_line = ""
    if resolved_at:
        resolved_line = f"**Resolved:** {safe_format_date(resolved_at, '%B %d, %Y at %I:%M %p')}\n"
...
async def auto_heal(error_type: str, error: Exception, context: dict[str, object] | None = None) -> bool:
    ...
    for attempt in range(1, _MAX_HEAL_RETRIES + 1):
        ...
        try:
            ...
            # ── Database / MongoDB ──────────────────────────────────
            if any(k in error_str for k in ("mongo", "database", "pymongo", "serverselection", "autoreconnect", "networktimeout", "connectionfailure")):
                logger.info("🔌 Attempting database auto-heal & reconnection...")
                if connect_db(is_reconnect=True):
                    logger.info("✅ Database connection restored & collections rebound!")
...
async def _can_send_auto_msg(uid: int) -> bool:
    ...
    if not last:
        return True
    # Handle datetime, numeric timestamp, and ISO string cleanly
    if hasattr(last, 'timestamp'):
        last_ts = last.timestamp()
    elif isinstance(last, (int, float)):
        last_ts = float(last)
    else:
        dt = safe_parse_dt(last)
        if dt:
...
async def broadcast_live_sync():
    """
    Background task — polls MSANodeDB.bot2_broadcasts every 10 seconds.

    Purpose:
      When Bot 2 sends, edits, or deletes a broadcast, this task detects the
      change via a lightweight fingerprint (id + index + first 60 chars of text).
      On change, it immediately re-renders every open Bot 1 dashboard message
      so users always see the latest announcement — no user action required.

    Fingerprint approach (no heavy watch / change stream needed):
      - Queries last 30 broadcasts: broadcast_id / index / message_text (60 chars).
      - Compares to last known fingerprint string.
      - If different → refreshes all active dashboard sessions.
      - If same → skips entirely (0 extra DB/Telegram work).

    Rate: 10-second poll interval (safe, well within Telegram rate limits).
    """
    last_fp: str = ""
    while True:
        try:
            ...
            # ── Step 1: Build fingerprint of current broadcasts ────────────────
            # Fetch only the minimal projection needed to detect any change.
            items = list(col_broadcasts.find(
                {"status": {"$ne": "scheduled"}},
                {"broadcast_id": 1, "index": 1, "message_text": 1, "status": 1, "sent_at": 1, "created_at": 1}
            ).sort([("sent_at", -1), ("index", -1)]).limit(30))
            fp = "|".join(
                f"{b.get('broadcast_id','')}/{b.get('index','')}/{b.get('status','')}/{b.get('sent_at','')}/{(b.get('message_text','')[:60])}"
                for b in items
            )
            ...
            if not _DASHBOARD_ACTIVE_MSGS:
                continue       # No active dashboards open — nothing to refresh

            # ── Step 2: Fetch fresh broadcasts and rebuild every open dashboard ─
            all_broadcasts = _fetch_deduplicated_broadcasts()
            total_bc       = len(all_broadcasts)
            ...
            for chat_id, sess in list(_DASHBOARD_ACTIVE_MSGS.items()):
                try:
                    ...
                    # ── Build announcement text for this user's current page ────
                    ann_text       = _build_ann_page(all_broadcasts, page)
                    dashboard_text = _build_dashboard_text(
                        user_name, display_msa_id, member_since, ann_text, None, msa_credits
                    )
                    # Hard cap: trim announcement if dashboard exceeds Telegram limit
                    if len(dashboard_text) > _DASH_CHAR_LIMIT:
                        excess = len(dashboard_text) - _DASH_CHAR_LIMIT + 5
                    ...
                    # ── Build inline keyboard (PREV / page counter / NEXT) ─────
                    if total_bc == 0:
                        ann_kb = None
                    elif total_bc == 1:
                        ann_kb = InlineKeyboardMarkup(inline_keyboard=[[
                    ...
                    # ── Push the refreshed card to Telegram ───────────────────
                    ...
                    if "message is not modified" in err_str:
                        ...
                        _DASHBOARD_ACTIVE_MSGS.pop(chat_id, None)  # stale session — remove
                    # Other transient errors: keep session alive, retry next cycle
